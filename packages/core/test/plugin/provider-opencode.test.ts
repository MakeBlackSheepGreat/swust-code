...
import { OpencodePlugin } from "@swust-code/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ SWUST_CODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
  ...
  it.effect("uses SWUST_CODE_API_KEY as credentials", () =>
    withEnv({ SWUST_CODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        yield* plugin.add(OpencodePlugin)
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ SWUST_CODE_API_KEY: undefined, CUSTOM_SWUST_CODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        yield* plugin.add(OpencodePlugin)
        const transform = yield* catalog.transform()
        yield* transform((catalog) => {
          const item = provider("opencode", { env: ["CUSTOM_SWUST_CODE_API_KEY"] })
