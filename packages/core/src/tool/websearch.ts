...
export class ConfigService extends Context.Service<ConfigService, Config>()("@swust-code/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SWUST_CODE_WEBSEARCH_PROVIDER === "exa" || process.env.SWUST_CODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SWUST_CODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SWUST_CODE_EXPERIMENTAL") || truthy("SWUST_CODE_ENABLE_EXA") || truthy("SWUST_CODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SWUST_CODE_ENABLE_PARALLEL") || truthy("SWUST_CODE_EXPERIMENTAL_PARALLEL"),
