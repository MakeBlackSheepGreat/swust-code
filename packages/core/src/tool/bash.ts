...
import { analyzeBashCommand, formatSafetyReport } from "./bash-safety"
      ...
      .register({
        [name]: Tool.make({
          ...
          execute: (input, context) =>
            Effect.gen(function* () {
              // Safety analysis
              const safety = analyzeBashCommand(input.command)
              if (safety.level === "dangerous") {
                return yield* Effect.fail(
                  new ToolFailure({
                    message: `Command blocked: ${formatSafetyReport(safety)}`,
                  }),
                )
              }
