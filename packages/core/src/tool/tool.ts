...
import { ToolDefinition, ToolFailure, ToolOutput, type ToolCall } from "@swust-code/llm"
...
export function isReadOnly(tool: AnyTool): boolean {
  return runtimes.get(tool)?.isReadOnly ?? false
}

export function isConcurrencySafe(tool: AnyTool): boolean {
  return runtimes.get(tool)?.isConcurrencySafe ?? false
}

export function isDestructive(tool: AnyTool): boolean {
  return runtimes.get(tool)?.isDestructive ?? true
}
