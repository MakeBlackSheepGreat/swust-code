 ...
 *
 * Ported from DevEco Code's document-validation/ patterns.
 */
...
export interface Section {
  ...
  children: Section[]  // mutable during parsing
...
// Section aliases for bilingual support
 ...
 * Strips bold markers, full-width spaces, annotation suffixes.
 */
export function normalizeSectionTitle(title: string): string {
  return title
    ...
    .replace(/\s*\*\(.*?\)\*/g, "") // annotation suffixes
    .trim()
    .toLowerCase()
}
 ...
 * Uses a stack-based algorithm that's code-fence-aware.
 */
export function parseMarkdownSections(content: string): ReadonlyArray<Section> {
  const lines = content.split("\n")
  let inCodeFence = false

  // Find all headings with their line ranges
...
/**
 * Collect all level-2 section titles from a section tree.
 */
...
/**
 * Check if a section title matches a required name (with aliases).
 */
function matchesSection(
  ...
  // Prefix match for colon-terminated titles
...
/**
 * Validate a document against a format template.
 */
