...
- Yerel olarak bunları projenizin `.swust-code/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/swust-code/tools/` içine yerleştirerek.
...
```ts title=".swust-code/tools/database.ts" {1}
...
```ts title=".swust-code/tools/math.ts"
import { tool } from "@swust-code/plugin"
...
```ts title=".swust-code/tools/bash.ts"
import { tool } from "@swust-code/plugin"
...
```ts title=".swust-code/tools/project.ts" {8}
import { tool } from "@swust-code/plugin"
...
```python title=".swust-code/tools/add.py"
import sys
...
```ts title=".swust-code/tools/python-add.ts" {10}
import { tool } from "@swust-code/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".swust-code/tools/add.py")
