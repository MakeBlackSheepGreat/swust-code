...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (veya `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\swust-code.jsonc` yapistirin
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/swust-code`
- **Linux**: `~/.cache/swust-code` dizinini silin (veya `rm -rf ~/.cache/swust-code` calistirin)
...
`swust-code.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
