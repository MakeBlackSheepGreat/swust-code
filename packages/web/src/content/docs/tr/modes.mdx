...
- `edit` - Planın kendisini detaylandırmak için `.swust-code/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`swust-code.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Projeye özel modlar için `.swust-code/modes/` veya genel modlar için `~/.config/swust-code/modes/`'de mod dosyaları oluşturun:
