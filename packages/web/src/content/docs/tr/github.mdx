...
opencode, GitHub iş akışınızla bütünleşir. Yorumunuzda `/swust-code` veya `/oc`'den bahsedin; opencode, GitHub Actions çalıştırıcınızdaki görevleri yürütecektir.
...
2. **İş akışını ekleyin**
   ...
   Aşağıdaki iş akışı dosyasını deponuzdaki `.github/workflows/swust-code.yml`'a ekleyin. `model`'de uygun `env` ve gerekli API anahtarlarını ayarladığınızdan emin olun.
...
| ----------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Bir soruna veya halkla ilişkilere yorum yapın | Yorumunuzda `/swust-code` veya `/oc`'den bahsedin. opencode bağlamı okur ve şubeler oluşturabilir, PR'leri açabilir veya yanıt verebilir.             |
| `pull_request_review_comment` | PR'deki belirli kod satırlarına yorum yapın   | Kodu incelerken `/swust-code` veya `/oc`'den bahsedin. opencode dosya yolunu, satır numaralarını ve fark içeriğini alır.                              |
