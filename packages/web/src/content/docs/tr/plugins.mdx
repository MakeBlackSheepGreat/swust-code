...
- `.swust-code/plugins/` - Proje düzeyinde eklentiler
- `~/.config/swust-code/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/swust-code/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/swust-code/swust-code.json`)
2. Project config (`swust-code.json`)
3. Global eklenti dizini (`~/.config/swust-code/plugins/`)
4. Proje eklenti dizini (`.swust-code/plugins/`)
