...
opencode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `swust-code auth login` kullanabilirsiniz. Bu `~/.local/share/swust-code/auth.json`'de saklanıyor.

```bash
swust-code auth login
```
