...
`~/.local/share/swust-code/auth.json` içinde.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `swust-code.json` kullanın:
...
`swust-code.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`swust-code.json` üzerinden özelleştirin:
...
4. Proje dizininizde `swust-code.json` dosyanızı oluşturun veya güncelleyin:
