...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.swust-code/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `swust-code.json` :
...
- Global : `~/.config/swust-code/modes/`
- Projet : `.swust-code/modes/`

```markdown title="~/.config/swust-code/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
```json title="swust-code.json" {4-14}
{
  "$schema": "https://opencode.ai/config.json",
  "mode": {
    "docs": {
...
Créez des fichiers de mode dans `.swust-code/modes/` pour les modes spécifiques au projet ou `~/.config/swust-code/modes/` pour les modes globaux :

```markdown title=".swust-code/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/swust-code/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
