...
en `~/.local/share/swust-code/auth.json`.
...
Pour une configuration spécifique au projet ou persistante, utilisez `swust-code.json` :
...
Fichier `swust-code.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `swust-code.json` :
...
4. Créez ou mettez à jour votre fichier `swust-code.json` dans le répertoire de votre projet :
