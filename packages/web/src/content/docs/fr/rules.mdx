...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/swust-code/AGENTS.md`. Ceci est appliqué à toutes les sessions opencode.
...
- **Règles globales** : `~/.claude/CLAUDE.md` (utilisé si aucun `~/.config/swust-code/AGENTS.md` n'existe)
...
2. **Fichier global** à `~/.config/swust-code/AGENTS.md`
3. **Fichier Claude Code** à `~/.claude/CLAUDE.md` (sauf si désactivé)

Le premier fichier correspondant gagne dans chaque catégorie. Par exemple, si vous possédez à la fois `AGENTS.md` et `CLAUDE.md`, seul `AGENTS.md` est utilisé. De même, `~/.config/swust-code/AGENTS.md` a priorité sur `~/.claude/CLAUDE.md`.

---

## Instructions personnalisées

Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `swust-code.json` ou dans le `~/.config/swust-code/swust-code.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de swust-code.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `swust-code.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `swust-code.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
