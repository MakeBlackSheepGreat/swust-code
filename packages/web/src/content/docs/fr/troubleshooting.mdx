...
- **macOS/Linux** : `~/.config/swust-code/swust-code.jsonc` (ou `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/swust-code/swust-code.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/swust-code`
- **Linux** : supprimez `~/.cache/swust-code` (ou exécutez `rm -rf ~/.cache/swust-code`)
...
Si votre `swust-code.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
