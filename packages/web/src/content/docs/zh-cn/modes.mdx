...
- `write` - 无法创建新文件
- `edit` - 无法修改现有文件，但位于 `.swust-code/plans/*.md` 的文件除外，用于详细说明计划本身
- `patch` - 无法应用补丁
- `bash` - 无法执行 shell 命令
...
在 `.swust-code/modes/` 中创建项目特定的模式文件，或在 `~/.config/swust-code/modes/` 中创建全局模式文件：
