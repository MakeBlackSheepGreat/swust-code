...
- `.swust-code/plugins/` - 项目级插件
- `~/.config/swust-code/plugins/` - 全局插件
...
**npm 插件**在启动时使用 Bun 自动安装。包及其依赖项会缓存在 `~/.cache/swust-code/node_modules/` 中。
...
1. 全局配置 (`~/.config/swust-code/swust-code.json`)
2. 项目配置 (`swust-code.json`)
3. 全局插件目录 (`~/.config/swust-code/plugins/`)
4. 项目插件目录 (`.swust-code/plugins/`)
