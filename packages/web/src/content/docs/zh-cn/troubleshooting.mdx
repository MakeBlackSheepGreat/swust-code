...
- **macOS/Linux**（旧版安装）: `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 粘贴 `~/.cache/swust-code`
- **Linux**: 删除 `~/.cache/swust-code`（或运行 `rm -rf ~/.cache/swust-code`）
...
如果你的 `swust-code.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。
