...
- `.swust-code/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/swust-code/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/swust-code/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/swust-code/swust-code.json`)
2. プロジェクト設定 (`swust-code.json`)
3. グローバルプラグインディレクトリ (`~/.config/swust-code/plugins/`)
4. プロジェクトプラグインディレクトリ (`.swust-code/plugins/`)
