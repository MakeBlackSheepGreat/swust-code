...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (または `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (古いインストール): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\opencode\swust-code.jsonc` を貼り付けます
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/swust-code` を貼り付け
- **Linux**: `~/.cache/swust-code` を削除します (または `rm -rf ~/.cache/swust-code` を実行します)。
...
`swust-code.json(c)` に `server` セクションが含まれている場合は、それを一時的に削除し、デスクトップアプリを再起動します。
