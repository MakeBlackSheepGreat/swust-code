...
- `edit` - 計画自体の詳細を示す `.swust-code/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`swust-code.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/swust-code/modes/`
- プロジェクト: `.swust-code/modes/`
...
モードファイルをプロジェクト固有モードの場合は `.swust-code/modes/` に、グローバルモードの場合は `~/.config/swust-code/modes/` に作成します。
