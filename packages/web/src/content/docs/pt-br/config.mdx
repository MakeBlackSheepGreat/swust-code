...
2. **Configuração global** (`~/.config/swust-code/swust-code.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`swust-code.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.swust-code`** - agentes, comandos, plugins
...
:::note
Os diretórios `.swust-code` e `~/.config/swust-code` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/swust-code/swust-code.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/swust-code/tui.json`.
...
Adicione `swust-code.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.swust-code`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.swust-code`, então ele **pode substituir** suas configurações.
...
Chaves legadas `theme`, `keybinds` e `tui` em `swust-code.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/swust-code/agents/` ou `.swust-code/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/swust-code/commands/` ou `.swust-code/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.swust-code/plugins/` ou `~/.config/swust-code/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
