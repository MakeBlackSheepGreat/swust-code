...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/swust-code`
- **Linux**: exclua `~/.cache/swust-code` (ou execute `rm -rf ~/.cache/swust-code`)
...
Se seu `swust-code.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
