...
O opencode integra-se ao seu fluxo de trabalho do GitHub. Mencione `/swust-code` ou `/oc` em seu comentário, e o opencode executará tarefas dentro do seu runner do GitHub Actions.
...
2. **Adicione o fluxo de trabalho**
   ...
   Adicione o seguinte arquivo de fluxo de trabalho em `.github/workflows/swust-code.yml` no seu repositório. Certifique-se de definir o `model` apropriado e as chaves de API necessárias em `env`.
...
| ----------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Comentário em um problema ou PR                     | Mencione `/swust-code` ou `/oc` em seu comentário. O opencode lê o contexto e pode criar branches, abrir PRs ou responder.               |
| `pull_request_review_comment` | Comentário em linhas de código específicas em um PR | Mencione `/swust-code` ou `/oc` enquanto revisa o código. O opencode recebe o caminho do arquivo, números das linhas e contexto do diff. |
