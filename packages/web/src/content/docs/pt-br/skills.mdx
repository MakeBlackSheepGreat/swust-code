...
- Configuração do projeto: `.swust-code/skills/<name>/SKILL.md`
- Configuração global: `~/.config/swust-code/skills/<name>/SKILL.md`
...
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.swust-code/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/swust-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.swust-code/skills/git-release/SKILL.md` assim:
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `swust-code.json`:
...
**Para agentes embutidos** (em `swust-code.json`):
