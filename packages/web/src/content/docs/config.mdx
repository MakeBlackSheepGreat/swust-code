...
2. **Global config** (`~/.config/swust-code/swust-code.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`swust-code.json` in project) - project-specific settings
5. **`.swust-code` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/swust-code/` on macOS) - admin-controlled
...
:::note
The `.swust-code` and `~/.config/swust-code` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global OpenCode config in `~/.config/swust-code/swust-code.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/swust-code/tui.json`.
...
Add `swust-code.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
modes, and plugins just like the standard `.swust-code` directory, and should
...
The custom directory is loaded after the global config and `.swust-code` directories, so it **can override** their settings.
...
Drop an `swust-code.json` or `swust-code.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/swust-code/` |
| Linux    | `/etc/swust-code/`                         |
...
On macOS, OpenCode reads managed preferences from the `ai.swust-code.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.swust-code.managed.plist`
2. `/Library/Managed Preferences/ai.swust-code.managed.plist`

The plist keys map directly to `swust-code.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.swust-code.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.swust-code.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.swust-code.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.swust-code</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Legacy `theme`, `keybinds`, and `tui` keys in `swust-code.json` are deprecated and automatically migrated when possible.
...
You can also define agents using markdown files in `~/.config/swust-code/agents/` or `.swust-code/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/swust-code/commands/` or `.swust-code/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.swust-code/plugins/` or `~/.config/swust-code/plugins/`. You can also load plugins from npm through the `plugin` option.
