...
2. **Global konfigurasjon** (`~/.config/swust-code/swust-code.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`swust-code.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.swust-code` kataloger** - agenter, kommandoer, plugins
...
:::note
`.swust-code` og `~/.config/swust-code` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/swust-code/swust-code.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `swust-code.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
moduser og plugins akkurat som standard `.swust-code` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.swust-code`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/swust-code/agents/` eller `.swust-code/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/swust-code/commands/` eller `.swust-code/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.swust-code/plugins/` eller `~/.config/swust-code/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
