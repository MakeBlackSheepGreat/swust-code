...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/swust-code`
- **Linux**: slett `~/.cache/swust-code` (eller kjør `rm -rf ~/.cache/swust-code`)
...
Hvis `swust-code.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
