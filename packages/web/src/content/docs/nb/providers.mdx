...
i `~/.local/share/swust-code/auth.json`.
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `swust-code.json`:
...
`swust-code.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `swust-code.json`:
...
4. Opprett eller oppdater `swust-code.json`-filen i prosjektkatalogen:
