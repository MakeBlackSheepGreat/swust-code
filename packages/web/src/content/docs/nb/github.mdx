...
OpenCode integreres med GitHub-arbeidsflyten din. Nevn `/swust-code` eller `/oc` i kommentaren din, og OpenCode vil utføre oppgaver i din GitHub Actions-løper.
...
2. **Legg til arbeidsflyten**
   ...
   Legg til følgende arbeidsflytfil til `.github/workflows/swust-code.yml` i repoen. Sørg for å sette riktig `model` og nødvendige API-nøkler i `env`.
...
| ----------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentar på sak eller PR               | Nevn `/swust-code` eller `/oc` i kommentaren din. OpenCode leser kontekst og kan opprette grener, åpne PR-er eller svare. |
| `pull_request_review_comment` | Kommenter spesifikke kodelinjer i en PR | Nevn `/swust-code` eller `/oc` mens du ser gjennom koden. OpenCode mottar filbane, linjenummer og diff-kontekst.          |
