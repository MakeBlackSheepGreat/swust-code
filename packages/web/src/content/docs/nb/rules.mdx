...
Du kan også ha globale regler i en `~/.config/swust-code/AGENTS.md`-fil. Dette blir brukt på alle OpenCode-økter.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (brukes hvis ingen `~/.config/swust-code/AGENTS.md` eksisterer)
...
Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, brukes bare `AGENTS.md`. På samme måte har `~/.config/swust-code/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Egendefinerte instruksjoner

Du kan spesifisere egendefinerte instruksjonsfiler i din `swust-code.json` eller den globale `~/.config/swust-code/swust-code.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
### Bruker swust-code.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `swust-code.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `swust-code.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
