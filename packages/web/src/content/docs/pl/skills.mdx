...
- Project config: `.swust-code/skills/<name>/SKILL.md`
- Global config: `~/.config/swust-code/skills/<name>/SKILL.md`
...
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.swust-code/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/swust-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.swust-code/skills/git-release/SKILL.md` w ten sposób:
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `swust-code.json`:
...
**Dla agentów wbudowanych** (w `swust-code.json`):
