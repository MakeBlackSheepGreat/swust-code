...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.swust-code/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `swust-code.json`:
...
- Globalnie: `~/.config/swust-code/modes/`
...
Utwórz pliki trybów w `.swust-code/modes/` dla trybów zapisanych dla projektu lub `~/.config/swust-code/modes/` dla trybów globalnych:
