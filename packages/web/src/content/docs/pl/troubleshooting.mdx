...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (lub `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **Linux**: usuń `~/.cache/swust-code` (lub uruchom `rm -rf ~/.cache/swust-code`)
...
Jeśli Twój `swust-code.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
