...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/swust-code/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.swust-code/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.swust-code/themes/*.json`
