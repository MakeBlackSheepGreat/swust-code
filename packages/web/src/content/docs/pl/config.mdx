...
2. **Konfiguracja globalna** (`~/.config/swust-code/swust-code.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`swust-code.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.swust-code`** - agenci, polecenia, umiejętności
...
:::note
W katalogach `.swust-code` i `~/.config/swust-code` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/swust-code/swust-code.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/swust-code/tui.json`.
...
Dodaj `swust-code.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.swust-code` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.swust-code`, więc **może być łączony** z ich zawartością.
...
Przestarzałe klucze `theme`, `keybinds` i `tui` w `swust-code.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/swust-code/agents/` lub `.swust-code/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/swust-code/commands/` lub `.swust-code/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.swust-code/plugins/` lub `~/.config/swust-code/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
