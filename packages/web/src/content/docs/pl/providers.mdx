...
w `~/.local/share/swust-code/auth.json`.
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `swust-code.json`:
...
`swust-code.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `swust-code.json`:
...
4. Utwórz lub zaktualizuj plik `swust-code.json` w katalogu projektu:
