...
OpenCode integruje się z wieloma dostawcami. Możesz użyć `swust-code auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/swust-code/auth.json`.
