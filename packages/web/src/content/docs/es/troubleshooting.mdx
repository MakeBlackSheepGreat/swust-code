...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (o `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/swust-code`
- **Linux**: eliminar `~/.cache/swust-code` (o ejecutar `rm -rf ~/.cache/swust-code`)
...
Si su `swust-code.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
