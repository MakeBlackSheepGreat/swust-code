...
- `.swust-code/plugins/` - Complementos a nivel de proyecto
- `~/.config/swust-code/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/swust-code/node_modules/`.
...
1. Configuración global (`~/.config/swust-code/swust-code.json`)
2. Configuración del proyecto (`swust-code.json`)
3. Directorio global de complementos (`~/.config/swust-code/plugins/`)
4. Directorio de complementos del proyecto (`.swust-code/plugins/`)
...
```json title=".swust-code/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
```ts title=".swust-code/plugins/my-plugin.ts"
...
```js title=".swust-code/plugins/example.js"
...
```js title=".swust-code/plugins/notification.js"
...
```javascript title=".swust-code/plugins/env-protection.js"
...
```javascript title=".swust-code/plugins/inject-env.js"
...
```ts title=".swust-code/plugins/custom-tools.ts"
import { type Plugin, tool } from "@swust-code/plugin"
...
```ts title=".swust-code/plugins/compaction.ts"
import type { Plugin } from "@swust-code/plugin"
...
```ts title=".swust-code/plugins/custom-compaction.ts"
import type { Plugin } from "@swust-code/plugin"
