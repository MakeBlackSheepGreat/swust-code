...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `swust-code.json`:
...
Archivo `swust-code.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `swust-code.json`:
...
4. Cree o actualice su archivo `swust-code.json` en el directorio de su proyecto:
