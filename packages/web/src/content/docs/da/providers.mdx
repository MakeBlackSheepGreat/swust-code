...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/swust-code/auth.json`.
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `swust-code.json`:
...
`swust-code.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
3. Indtast API-nøglen for udbyderen.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in swust-code.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Opret eller opdater `swust-code.json`-filen i projektkataloget:
