...
2. **Global config** (`~/.config/swust-code/swust-code.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`swust-code.json` i projekt) - projektspecifikke indstillinger
5. **`.swust-code` mapper** - agent, kommandoer, plugins
...
:::note
`.swust-code` og `~/.config/swust-code` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale OpenCode-konfiguration i `~/.config/swust-code/swust-code.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `swust-code.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
modes og plugins ligesom standard `.swust-code` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.swust-code` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/swust-code/agents/` eller `.swust-code/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/swust-code/commands/` eller `.swust-code/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.swust-code/plugins/` eller `~/.config/swust-code/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
