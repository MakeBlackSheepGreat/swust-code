...
Du kan også ha globale regler i en `~/.config/swust-code/AGENTS.md`-fil. Dette blir brugt på alle opencode-sessioner.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (bruges hvis ingen `~/.config/swust-code/AGENTS.md` eksisterer)
...
2. **Global fil** på `~/.config/swust-code/AGENTS.md`
3. **Claude Code-fil** på `~/.claude/CLAUDE.md` (med mindre de er deaktivert)

Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, bruges bare `AGENTS.md`. På samme måte har `~/.config/swust-code/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Brugerdefinerede instruktioner

Du kan spesifisere egendefinerte instruksjonsfiler i din `swust-code.json` eller den globale `~/.config/swust-code/swust-code.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af swust-code.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `swust-code.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `swust-code.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
