...
OpenCode integreres med din GitHub arbejdsgang. Nævn `/swust-code` eller `/oc` i din kommentar, og OpenCode vil udføre opgaver i din GitHub Actions-løber.
...
2. **Tilføj arbejdsgangen**
   ...
   Tilføj følgende workflow-fil til `.github/workflows/swust-code.yml` i din repo. Sørg for at indstille de relevante `model` og nødvendige API-nøgler i `env`.
...
| ----------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentarer og problemer eller PR       | Nævn `/swust-code` eller `/oc` i din kommentar. OpenCode læser kontekst og kan oprette brancher, åbne PR'er eller svare.  |
| `pull_request_review_comment` | Kommenter specifikke kodelinjer i en PR | Nævn `/swust-code` eller `/oc`, mens du gennemgår koden. OpenCode modtager filsti, linjenumre og diff-kontekst.           |
