...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (eller `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/swust-code`
- **Linux**: slet `~/.cache/swust-code` (eller kør `rm -rf ~/.cache/swust-code`)
...
Hvis `swust-code.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
