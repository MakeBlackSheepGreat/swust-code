...
- `edit` - Cannot modify existing files, except for files located at `.swust-code/plans/*.md` to detail the plan itself
...
Configure modes in your `swust-code.json` config file:
...
Create mode files in `.swust-code/modes/` for project-specific modes or `~/.config/swust-code/modes/` for global modes:
