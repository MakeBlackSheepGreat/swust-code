...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc`（或 `~/.config/swust-code/swust-code.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 貼上 `~/.cache/swust-code`
- **Linux**: 刪除 `~/.cache/swust-code`（或執行 `rm -rf ~/.cache/swust-code`）
...
如果你的 `swust-code.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。
