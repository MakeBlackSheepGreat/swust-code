...
- `write` - 無法建立新檔案
- `edit` - 無法修改現有檔案，但位於 `.swust-code/plans/*.md` 的檔案除外，用於詳細說明計畫本身
- `patch` - 無法套用補丁
- `bash` - 無法執行 shell 指令
...
在 `.swust-code/modes/` 中建立專案特定的模式檔案，或在 `~/.config/swust-code/modes/` 中建立全域模式檔案：
