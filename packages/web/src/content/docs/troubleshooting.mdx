...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (or `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (older installs): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **Linux**: delete `~/.cache/swust-code` (or run `rm -rf ~/.cache/swust-code`)
...
If your `swust-code.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
