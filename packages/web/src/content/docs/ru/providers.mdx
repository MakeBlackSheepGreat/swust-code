...
в `~/.local/share/swust-code/auth.json`.
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `swust-code.json`:
...
`swust-code.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Настройте через `swust-code.json`:
...
4. Создайте или обновите файл `swust-code.json` в каталоге вашего проекта:
