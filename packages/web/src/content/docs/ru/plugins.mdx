...
- `.swust-code/plugins/` – плагины уровня проекта.
- `~/.config/swust-code/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/swust-code/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/swust-code/swust-code.json`)
2. Конфигурация проекта (`swust-code.json`)
3. Глобальный каталог плагинов (`~/.config/swust-code/plugins/`)
4. Каталог плагинов проекта (`.swust-code/plugins/`)
