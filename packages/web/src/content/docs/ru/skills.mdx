...
- Конфигурация проекта: `.swust-code/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/swust-code/skills/<name>/SKILL.md`.
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.swust-code/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/swust-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.swust-code/skills/git-release/SKILL.md` следующим образом:
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `swust-code.json`:
...
**Для встроенных агентов** (в формате `swust-code.json`):
