...
2. **Globalna konfiguracija** (`~/.config/swust-code/swust-code.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`swust-code.json` u projektu) - postavke specifične za projekat
5. **`.swust-code` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.swust-code` i `~/.config/swust-code` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/swust-code/swust-code.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/swust-code/tui.json`.
...
Dodajte `swust-code.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.swust-code` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.swust-code`, tako da **može nadjačati** njihove postavke.
...
Stari `theme`, `keybinds`, i `tui` ključevi u `swust-code.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/swust-code/agents/` ili `.swust-code/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/swust-code/commands/` ili `.swust-code/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.swust-code/plugins/` ili `~/.config/swust-code/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
