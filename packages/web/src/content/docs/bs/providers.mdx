...
u `~/.local/share/swust-code/auth.json`.
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `swust-code.json`:
...
`swust-code.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `swust-code.json`:
...
4. Kreirajte ili ažurirajte svoju `swust-code.json` datoteku u direktoriju projekta:
