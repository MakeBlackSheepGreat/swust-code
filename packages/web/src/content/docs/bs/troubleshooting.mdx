...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (ili `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/swust-code`
- **Linux**: obrišite `~/.cache/swust-code` (ili pokrenite `rm -rf ~/.cache/swust-code`)
...
Ako vaš `swust-code.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
