...
2. **Globale Konfiguration** (`~/.config/swust-code/swust-code.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`swust-code.json` im Projekt) – projektspezifische Einstellungen
5. **`.swust-code` Verzeichnisse** – Agenten, Befehle, Plugins
...
:::note
Die Verzeichnisse `.swust-code` und `~/.config/swust-code` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/swust-code/swust-code.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `swust-code.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Modi und Plugins genau wie das Standardverzeichnis `.swust-code` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.swust-code` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/swust-code/agents/` oder `.swust-code/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/swust-code/commands/` oder `.swust-code/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.swust-code/plugins/` oder `~/.config/swust-code/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
