...
Dabei werden passende `skills/*/SKILL.md` in `.swust-code/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/swust-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.swust-code/skills/git-release/SKILL.md` zum Beispiel so:
...
Steuere in `swust-code.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `swust-code.json`):
