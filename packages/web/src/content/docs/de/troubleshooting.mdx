...
- **macOS/Linux**: `~/.local/share/swust-code/log/`
...
- **macOS/Linux**: `~/.local/share/swust-code/`
...
- **macOS/Linux**: `~/.config/swust-code/swust-code.jsonc` (oder `~/.config/swust-code/swust-code.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **Globale Plugins**
  - **macOS/Linux**: `~/.config/swust-code/plugins/`
...
- **Projekt-Plugins** (nur bei projektspezifischer Konfig)
  - `<your-project>/.swust-code/plugins/`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/swust-code`
- **Linux**: loesche `~/.cache/swust-code` (oder fuehre aus: `rm -rf ~/.cache/swust-code`)
...
Wenn deine `swust-code.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
1. **Probleme auf GitHub melden**
   ...
   [**github.com/MakeBlackSheepGreat/swust-code/issues**](https://github.com/MakeBlackSheepGreat/swust-code/issues)
