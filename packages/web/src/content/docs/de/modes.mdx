...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.swust-code/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `swust-code.json`-Konfigurationsdatei:
...
Erstellen Sie Modusdateien in `.swust-code/modes/` für projektspezifische Modi oder `~/.config/swust-code/modes/` für globale Modi:
