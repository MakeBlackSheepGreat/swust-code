...
- `.swust-code/plugins/` – Plugins auf Projektebene
- `~/.config/swust-code/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/swust-code/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/swust-code/swust-code.json`)
2. Projektkonfiguration (`swust-code.json`)
3. Globales Plugin-Verzeichnis (`~/.config/swust-code/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.swust-code/plugins/`)
