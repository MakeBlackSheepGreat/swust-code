...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/swust-code/themes/*.json` oder `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.swust-code/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.swust-code/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/swust-code/themes
vim ~/.config/swust-code/themes/my-theme.json
```

Und fuer projektspezifische Themes:

```bash no-frame
mkdir -p .swust-code/themes
vim .swust-code/themes/my-theme.json
```
