...
Du kannst auch globale Regeln in `~/.config/swust-code/AGENTS.md` speichern.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/swust-code/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/swust-code/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `swust-code.json` oder in `~/.config/swust-code/swust-code.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `swust-code.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `swust-code.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
