...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/swust-code/swust-code.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/swust-code`
- **Linux**: elimina `~/.cache/swust-code` (oppure esegui `rm -rf ~/.cache/swust-code`)
...
Se il tuo `swust-code.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
