...
- `edit` - Non puo modificare file esistenti, tranne i file in `.swust-code/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `swust-code.json`:
...
- Globale: `~/.config/swust-code/modes/`
- Progetto: `.swust-code/modes/`
...
Crea file di modalita in `.swust-code/modes/` per modalita specifiche del progetto oppure in `~/.config/swust-code/modes/` per modalita globali:
