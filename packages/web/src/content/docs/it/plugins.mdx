...
- `.swust-code/plugins/` - plugin a livello progetto
- `~/.config/swust-code/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/swust-code/node_modules/`.
...
1. Config globale (`~/.config/swust-code/swust-code.json`)
2. Config di progetto (`swust-code.json`)
3. Directory plugin globale (`~/.config/swust-code/plugins/`)
4. Directory plugin di progetto (`.swust-code/plugins/`)
