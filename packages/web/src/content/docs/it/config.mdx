...
2. **Config globale** (`~/.config/swust-code/swust-code.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`swust-code.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.swust-code`** - agenti, comandi, plugin
...
:::note
Le directory `.swust-code` e `~/.config/swust-code` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di OpenCode in `~/.config/swust-code/swust-code.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/swust-code/tui.json`.
...
Aggiungi `swust-code.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.swust-code` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.swust-code`, quindi **puo sovrascrivere** le loro impostazioni.
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `swust-code.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi anche definire agenti usando file markdown in `~/.config/swust-code/agents/` o `.swust-code/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/swust-code/commands/` o `.swust-code/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.swust-code/plugins/` o `~/.config/swust-code/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
