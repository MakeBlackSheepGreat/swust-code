...
2. **Directory di configurazione utente** - in `~/.config/swust-code/themes/*.json` o `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.swust-code/themes/*.json`
4. **Directory di lavoro corrente** - in `./.swust-code/themes/*.json`
