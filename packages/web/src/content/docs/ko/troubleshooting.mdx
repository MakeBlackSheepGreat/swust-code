...
-**macOS/리눅스**: `~/.local/share/swust-code/log/`
...
-**macOS/리눅스**: `~/.local/share/swust-code/`
...
-**macOS/리눅스**: `~/.config/swust-code/swust-code.jsonc` (또는 `~/.config/swust-code/swust-code.json`) -**macOS/Linux** (외부 설치): `~/.local/share/swust-code/swust-code.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\swust-code.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/swust-code/plugins/`
...
-**macOS**: 찾기 -> `Cmd+Shift+G` -> 붙여넣기 `~/.cache/swust-code`

- **Linux**: `~/.cache/swust-code` 삭제 (또는 `rm -rf ~/.cache/swust-code` 실행)
...
`swust-code.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.
