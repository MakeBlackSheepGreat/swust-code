...
2. **الإعدادات العامة** (`~/.config/swust-code/swust-code.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`swust-code.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.swust-code`** - الوكلاء، الأوامر، الإضافات
...
:::note
يستخدم الدليلان `.swust-code` و`~/.config/swust-code` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
ضع إعدادات OpenCode العامة في `~/.config/swust-code/swust-code.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `swust-code.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
الدليل القياسي `.swust-code`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.swust-code`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/swust-code/agents/` أو `.swust-code/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/swust-code/commands/` أو `.swust-code/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
ضع ملفات الإضافات في `.swust-code/plugins/` أو `~/.config/swust-code/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
