...
في `~/.local/share/swust-code/auth.json`.
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `swust-code.json`:
...
`swust-code.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
4. أنشئ أو حدّث ملف `swust-code.json` في دليل مشروعك:
