...
- إعدادات المشروع: `.swust-code/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/swust-code/skills/<name>/SKILL.md`
...
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.swust-code/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/swust-code/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.swust-code/skills/git-release/SKILL.md` بهذا الشكل:
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `swust-code.json`:
...
**للوكلاء المدمجين** (في `swust-code.json`):
