...
It loads any matching `skills/*/SKILL.md` in `.swust-code/` and any matching `.claude/skills/*/SKILL.md` or `.agents/skills/*/SKILL.md` along the way.

Global definitions are also loaded from `~/.config/swust-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md`, and `~/.agents/skills/*/SKILL.md`.
...
Create `.swust-code/skills/git-release/SKILL.md` like this:
...
Control which skills agents can access using pattern-based permissions in `swust-code.json`:
...
**For built-in agents** (in `swust-code.json`):
