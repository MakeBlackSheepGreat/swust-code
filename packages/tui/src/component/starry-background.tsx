    ...
    if (m) {
      ...
      if (elapsed >= 0 && elapsed <= METEOR_DURATION) {
        ...
        for (const [key, val] of cellAcc) {
          ...
          const color = tint(theme.background, tint(beamGlow, beamCore, headBlend), Math.max(0.02, fade))
    ...
    f.grid.forEach((row, y) => {
      row.forEach((cell, x) => {
        ...
        const baseColor = dark
          ? tint(theme.background, RGBA.fromInts(147, 197, 253), Math.min(1, b * 1.05))
          : tint(theme.background, RGBA.fromInts(30, 64, 175), Math.min(1, b * 0.95))
        const starColor =
          peak > 0
            ? tint(baseColor, dark ? RGBA.fromInts(255, 255, 255) : RGBA.fromInts(14, 116, 144), peak * (dark ? 0.65 : 0.5))
            : baseColor
