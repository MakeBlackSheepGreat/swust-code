...
const SWUST_CODE_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(SWUST_CODE_MODE_KEY, SWUST_CODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(SWUST_CODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(SWUST_CODE_MODE_KEY, stack.at(-1)?.mode ?? SWUST_CODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? SWUST_CODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(SWUST_CODE_MODE_KEY, undefined)
