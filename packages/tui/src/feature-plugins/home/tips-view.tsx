...
function pickWeighted(keys: readonly string[]): string {
  const weights = keys.map((key) => PRIORITY_WEIGHTS[key] ?? 1)
...
export function Tips(props: { connected?: boolean }) {
  ...
  const allKeys = createMemo<readonly string[]>(() =>
    props.connected === false ? ["tui.tips.connect"] : [...TIP_KEYS, platformSuspendKey],
  )
  ...
  const parts = createMemo(() => parse(language.t(key(), { count: themeCount })))
