...
import { EventSequenceTable } from "@swust-code/core/event/sql"
...
import { WorkspaceV2 } from "@swust-code/core/workspace"
import { WorkspaceTable } from "@swust-code/core/control-plane/workspace.sql"
...
const originalEnv = {
  SWUST_CODE_AUTH_CONTENT: process.env.SWUST_CODE_AUTH_CONTENT,
  SWUST_CODE_EXPERIMENTAL_WORKSPACES: process.env.SWUST_CODE_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SWUST_CODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.SWUST_CODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.SWUST_CODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.SWUST_CODE_EXPERIMENTAL_WORKSPACES).toBe("true")
