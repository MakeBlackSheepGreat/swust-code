...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "swust-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "swust-code.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "swust-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "swust-code.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["swust-code.json", "swust-code.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "swust-code.json"),
        path.join(project, "swust-code.jsonc"),
      ])
    })
  })
