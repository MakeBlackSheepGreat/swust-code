...
// the live router + a real key in ~/.config/swust-code/swust-code.json). Run with:
...
function realConfig() {
  const home = path.join(os.homedir(), ".config", "swust-code", "swust-code.json")
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .swust-code/workflows.
            const wfDir = path.join(dir, ".swust-code", "workflows")
