...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.SWUST_CODE_SERVER_PASSWORD = input?.password
      Flag.SWUST_CODE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".swust-code", "skills", "project-rest-skill", "SKILL.md"),
      `---
