...
const original = {
  ...
  envPassword: process.env.SWUST_CODE_SERVER_PASSWORD,
  envUsername: process.env.SWUST_CODE_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.SWUST_CODE_SERVER_PASSWORD
  else process.env.SWUST_CODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.SWUST_CODE_SERVER_USERNAME
  else process.env.SWUST_CODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.SWUST_CODE_SERVER_PASSWORD = auth.password
  Flag.SWUST_CODE_SERVER_USERNAME = auth.username
  process.env.SWUST_CODE_SERVER_PASSWORD = auth.password
  process.env.SWUST_CODE_SERVER_USERNAME = auth.username
