...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      SWUST_CODE_EXPERIMENTAL_WORKSPACES: Flag.SWUST_CODE_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.SWUST_CODE_EXPERIMENTAL_WORKSPACES = original.SWUST_CODE_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
