...
import { isRecord } from "@swust-code/tui/util/record"
...
import { TuiConfig } from "@swust-code/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@swust-code/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.swust-code`
  // folders between cwd and home, and SWUST_CODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SWUST_CODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SWUST_CODE_TUI_CONFIG override, if set.
  if (Flag.SWUST_CODE_TUI_CONFIG) {
    const configFile = Flag.SWUST_CODE_TUI_CONFIG
  ...
  // 4. `.swust-code` directories (and SWUST_CODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".swust-code") || dir === Flag.SWUST_CODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".swust-code") && dir !== Flag.SWUST_CODE_CONFIG_DIR) continue
