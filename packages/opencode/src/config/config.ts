...
import type { ConsoleState } from "@swust-code/core/v1/config/console-state"
...
import { RemoteAuthError } from "@swust-code/core/v1/config/error"
import { ConfigPermissionV1 } from "@swust-code/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@swust-code/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["swust-code.jsonc", "swust-code.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SWUST_CODE_CONFIG && !Flag.SWUST_CODE_CONFIG_DIR && !Flag.SWUST_CODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "swust-code.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "swust-code.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "SWUST_CODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.SWUST_CODE_CONFIG) {
          yield* merge(Flag.SWUST_CODE_CONFIG, yield* loadFile(Flag.SWUST_CODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SWUST_CODE_CONFIG })
        }

        if (!Flag.SWUST_CODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("swust-code", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.SWUST_CODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SWUST_CODE_CONFIG_DIR", { path: Flag.SWUST_CODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".swust-code") || dir === Flag.SWUST_CODE_CONFIG_DIR) {
            for (const file of ["swust-code.json", "swust-code.jsonc"]) {
          ...
          // Auto-discovered plugins under `.swust-code/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.SWUST_CODE_CONFIG_CONTENT) {
          const source = "SWUST_CODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.SWUST_CODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from SWUST_CODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["SWUST_CODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("SWUST_CODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.SWUST_CODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.SWUST_CODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("SWUST_CODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.SWUST_CODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.SWUST_CODE_DISABLE_PRUNE) {
