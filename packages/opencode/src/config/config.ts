...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in swust-code config are deprecated; move them to tui.json", { path: source })
  return copy
}
  ...
  memory: Schema.optional(
    Schema.Struct({
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false. Note: when enabled, every swust-code agent (build/explore/subagents) can search these memories via the builtin `memory` tool — including CC's `type: user` (your role/preferences) and `type: feedback` (your guidance) categories. CC originally writes them for future CC sessions; flipping this on widens the consumer set to swust-code agents on the same machine. Leave disabled (default) if you don't want personal context recallable from a prompt-injection-vulnerable agent.",
      }),
    }),
  ),
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "swust-code.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "swust-code.jsonc"))),
      )
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Flag.SWUST_CODE_CONFIG) {
          yield* merge(Flag.SWUST_CODE_CONFIG, yield* loadFile(Flag.SWUST_CODE_CONFIG))
          log.debug("loaded custom config", { path: Flag.SWUST_CODE_CONFIG })
        }
        ...
        if (Flag.SWUST_CODE_CONFIG_DIR) {
          log.debug("loading config from SWUST_CODE_CONFIG_DIR", { path: Flag.SWUST_CODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .swust-code commands override on name collision.
        ...
        if (process.env.SWUST_CODE_CONFIG_CONTENT) {
          ...
          log.debug("loaded custom config from SWUST_CODE_CONFIG_CONTENT")
        }
        ...
        if (!Flag.SWUST_CODE_DISABLE_CLAUDE_CODE_MCP) {
