...
import type { Message, OpencodeClient, SessionMessageResponse } from "@swust-code/sdk/v2"
...
export class Service extends Context.Service<Service, Interface>()("@swust-code/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `swust-code auth login` in the terminal",
      name: "Login with SWUST Code",
      id: AuthMethodID,
    }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "SWUST Code service failure", service })
}
