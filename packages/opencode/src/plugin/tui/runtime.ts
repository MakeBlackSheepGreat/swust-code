...
import { errorData, errorMessage } from "@swust-code/tui/util/error"
...
import { hasTheme, upsertTheme } from "@swust-code/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@swust-code/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@swust-code/core/v1/config/plugin"
import { createCommandShim } from "@swust-code/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@swust-code/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".swust-code"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".swust-code", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".swust-code", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.SWUST_CODE_PURE ? [] : pluginOrigins
    if (Flag.SWUST_CODE_PURE && pluginOrigins.length) {
    }
