 ...
 * TODO: lift to swust-code.json config (e.g. session.maxGoalReact).
 */
...
const OUTPUT_LENGTH_CONTINUATION_LIMIT = Flag.SWUST_CODE_OUTPUT_LENGTH_CONTINUATION_LIMIT
const INVALID_OUTPUT_CONTINUATION_LIMIT = Flag.SWUST_CODE_INVALID_OUTPUT_CONTINUATION_LIMIT

const log = Log.create({ service: "session.prompt" })

function isExtensionPath(filePath: string): boolean {
  return /\/\.swust-code\/(tools?|skills?|hooks?)\//.test(filePath)
}
    ...
    const runLoop: (sessionID: SessionID, agentID?: string, task_id?: string) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
      ...
      function* (sessionID: SessionID, agentID?: string, task_id?: string) {
        ...
        const publishAgentRequest = (phase: string, taskType: string) =>
          bus
            .publish(Metrics.AgentRequest, {
              ...
              phase,
              task_type: taskType,
              surface: Flag.SWUST_CODE_CLIENT,
        ...
        while (true) {
          ...
          // (explore/general/etc) spawned via swust-code's shared-sessionID
