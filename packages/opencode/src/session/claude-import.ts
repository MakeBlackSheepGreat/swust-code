...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in swust-code, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's swust-code-native continuation messages in the same session.
          ...
          // Preserve swust-code-owned metadata on re-sync: keep any user rename
