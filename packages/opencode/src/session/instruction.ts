...
const FILES = [
  "AGENTS.md",
  ...(Flag.SWUST_CODE_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.SWUST_CODE_CONFIG_DIR) {
    files.push(path.join(Flag.SWUST_CODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.SWUST_CODE_DISABLE_CLAUDE_CODE_PROMPT) {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.SWUST_CODE_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no SWUST_CODE_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.SWUST_CODE_CONFIG_DIR, Flag.SWUST_CODE_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
