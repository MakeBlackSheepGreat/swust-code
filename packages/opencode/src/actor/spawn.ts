 ...
 * TODO: lift to swust-code.json config (e.g. actor.maxPreReact) and add per-hook
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const releaseSpawnRef = spawnRef.push(impl)
    yield* Effect.addFinalizer(() => Effect.sync(releaseSpawnRef))
    return impl
  }),
)
