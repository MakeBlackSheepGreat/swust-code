...
//
// Renders the full SWUST Code entry logo and a compact [S] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "SWUST Code", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `swust-code run -i -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
