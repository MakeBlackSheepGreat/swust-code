...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const SWUST_BLUE = RGBA.fromInts(30, 64, 175)
  const LOGO_GRAY = RGBA.fromInts(160, 160, 160)
          ...
          if (labelRow) {
            return (
              <box flexDirection="row" gap={1}>
                <text fg={LOGO_GRAY} selectable={false}>{line}</text>
                <text fg={LOGO_GRAY} selectable={false}>{ctx.shape.right[index()]}</text>
              </box>
            )
          }
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), props.ink ?? SWUST_BLUE, true, 0, frame(), dusk(), idleState())}
              </box>
              <box flexDirection="row">
                {renderLine(
