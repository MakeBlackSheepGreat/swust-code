...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.SWUST_CODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.SWUST_CODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.SWUST_CODE_SERVER_USERNAME ?? "swust-code"
