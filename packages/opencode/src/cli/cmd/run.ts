...
//      swust-code server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@swust-code/sdk/v2"
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running swust-code server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to SWUST_CODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to SWUST_CODE_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
      ...
      .option("goal", {
        type: "string",
        describe: "set an autonomous goal - agent works until goal is met or judged impossible",
      })
