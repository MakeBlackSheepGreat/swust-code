...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-swust-code-directory")
  out.delete("x-swust-code-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-swust-code-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-swust-code-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
