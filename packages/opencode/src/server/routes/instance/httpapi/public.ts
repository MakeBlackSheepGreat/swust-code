...
import { SwustCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for swust-code serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SwustCodeHttpApi)
...
export const PublicApi = SwustCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "SWUST Code",
    version: "1.0.0",
    description: "SWUST Code API",
