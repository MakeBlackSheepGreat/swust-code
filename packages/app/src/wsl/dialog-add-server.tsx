...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const opencodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.swustCodeChecks[distro] ?? null
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.swustCodeChecks[distro]) {
  ...
  const opencodeMessage = createMemo(() => {
    ...
    if (opencodeReady()) {
      return distro
        ? language.t("wsl.onboarding.swust-codeReadyIn", { distro })
        : language.t("wsl.onboarding.swust-codeReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.swust-code"),
        state:
          active === step
            ? "current"
            : step === "wsl"
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "opencode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.swust-code")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
